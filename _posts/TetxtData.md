...
### Use NLP to transform text data into numeric values
In order to perform predictive models on a dataset, the text data must first be transformed into numeric values. We are basically taking words and organizing them in a language that can be processed by a computer. 

![Alternate image text](/images/NLP/TextCommunication.png)

I used pd.get_dummies which is a one-hot-encoding process to transform Country, Province, and Variety into a 1 or 0. I then used TF-IDF from sklearn to transform the wine descriptions into weighted tokens.
